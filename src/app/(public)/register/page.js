...
import { Mail, Lock, User, Image, Check, ArrowRight, CarFront, AlertCircle } from "lucide-react";
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!isPasswordValid) {
      setErrorMsg("Please satisfy all password validation requirements before submitting.");
    ...
    try {
      ...
      if (error) {
        setErrorMsg(error.message || "Registration failed. Please check your inputs and try again.");
        ...
        {errorMsg && (
          <div className="p-4 bg-danger/10 border border-danger/20 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-danger">Registration Error</p>
